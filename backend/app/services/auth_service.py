...
def normalize_permissions(permissions: Iterable[str]) -> list[str]:
    ...
    for permission in permissions:
        ...
        normalized.add(permission.replace(".", ":"))
    # Permissões de leitura comuns para o painel operacional.
...
class AuthService:
    ...
    async def _serialize_user(self, user: User, role: Role, tenant: Tenant, permissions: list[str]) -> UserOut:
        return UserOut(
            id=user.id,
            email=user.email,
            tenant_id=user.tenant_id,
            status=str(user.status.value if hasattr(user.status, "value") else user.status),
            role=RoleOut(id=role.id, name=role.name, permissions=permissions),
            tenant=TenantOut(id=tenant.id, name=tenant.name, active=tenant.active),
        )

    async def authenticate(self, email: str, password: str, tenant_id: Optional[UUID] = None) -> LoginResponse:
        stmt = (
        ...
        if user_status != UserStatus.ACTIVE.value or not tenant.active:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User inactive or tenant disabled")

        if not verify_password(password, user.password_hash):
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")

        permissions = await self._get_permissions(role.id)
        access_token = create_access_token(str(user.id), str(user.tenant_id), permissions)
        refresh_token = create_refresh_token(str(user.id), str(user.tenant_id), permissions)
    ...
    async def refresh(self, refresh_token: str) -> TokenResponse:
        ...
        access_token = create_access_token(str(user_id), str(tenant_id), permissions)
