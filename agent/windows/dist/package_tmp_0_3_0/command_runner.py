...
def collect_security_inventory(payload: dict[str, Any] | None = None) -> CommandResult:
    ...
    hotfix_limit = _safe_int(payload.get("hotfix_limit"), default=30, minimum=1, maximum=300)
    software_limit = _safe_int(payload.get("software_limit"), default=50, minimum=1, maximum=500)
...
try {{
    $hotfixes = Get-HotFix |
        ...
        Select-Object -First {hotfix_limit} |
        ForEach-Object {{
            [ordered]@{{
...
try {{
    ...
    $recentSoftware = $recentSoftware |
        ...
        ForEach-Object {{ $_.Group[0] }} |
        Sort-Object name |
        Select-Object -First {software_limit}
}} catch {{}}
