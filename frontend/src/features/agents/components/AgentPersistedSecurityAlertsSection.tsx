import {
  useAgentActiveSecurityAlerts,
  useAgentSecurityAlertsSummary,
...
export function AgentPersistedSecurityAlertsSection({ agentId }: Props) {
  const summaryQuery = useAgentSecurityAlertsSummary(agentId);
  const alertsQuery = useAgentActiveSecurityAlerts(agentId, {
    limit: 10,
  });
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-950">
            Alertas ativos deste agente
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            Alertas persistidos a partir da última coleta de segurança deste agente.
          </p>
        </div>
      ...
      {hasError ? (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          Não foi possível carregar os alertas ativos deste agente.
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          ...
          {alerts.length === 0 ? (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
              Nenhum alerta ativo encontrado para este agente.
            </div>
          ) : (
